...
)
{
	...
	if (!parserResult || Error::hasErrorsWarningsOrInfos(errorReporter.errors()) || !analyzerResult)
	...
	if (Error::hasErrorsWarningsOrInfos(errorReporter.errors()))
		reportError("Failed to analyze inline assembly block.");

	solAssert(!Error::hasErrorsWarningsOrInfos(errorReporter.errors()), "Failed to analyze inline assembly block.");
