...
inline bool operator==(SortId first, SortId second) { return first.id == second.id; }

struct SortPairHash
{
	std::size_t operator()(std::pair<SortId, SortId> const& pair) const;
};
...
struct SMTLibType {
	Kind const kind;
	SortId const id;
	SMTLibType(Kind kind, SortId id): kind(kind), id(id) {}
...
struct SMTLibSort : public SMTLibType
{
	...
	SMTLibSort(
		...
		SortId _id
	): SMTLibType(kind, _id), name(_name), args(std::move(_args)) {}
};

struct TupleType : public SMTLibType
{
	std::string const m_name;
	std::vector<std::pair<std::string,SortId>> m_accessors;

	TupleType(std::string_view _name, std::vector<std::pair<std::string,SortId>> _accessors, SortId _id)
	: SMTLibType(Kind::Tuple, _id), m_name(_name), m_accessors(std::move(_accessors)) {}
};
...
public:
	...
	void setDeclared(std::string const& _name, SortPointer const& _sort);
	SortPointer getDeclaredSort(std::string const& _name) const;

	// FIXME: Should not be exposed like this!
	auto const& variables() const { return m_functions; }

	SortId resolve(SortPointer const& _sort);
	SortPointer unresolve(SortId) const;
	...
	void setTupleDeclarationCallback(TupleDeclarationCallback callback);
private:
	...
	using datatypes_t = std::unordered_set<std::string>;
