...
{
	std::string vars("(");
	for (auto const& [name, sort]: m_context.variables())
	{
		solAssert(sort, "");
...
std::optional<SortPointer> CHCSmtLib2Interface::ScopedParser::lookupKnownTupleSort(std::string const& name) const
{
	return m_context.getTupleType(name);
}

SortPointer CHCSmtLib2Interface::ScopedParser::toSort(SMTLib2Expression const& expr)
{
	if (isAtom(expr))
	{
