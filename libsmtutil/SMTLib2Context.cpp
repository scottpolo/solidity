...
std::size_t SortPairHash::operator()(std::pair<SortId, SortId> const& pair) const
{
	std::size_t seed = 0;
	boost::hash_combine(seed, pair.first.id);
	boost::hash_combine(seed, pair.second.id);
	return seed;
}
...
void SMTLib2Context::setDeclared(std::string const& _name, SortPointer const& _sort)
{
...
void SMTLib2Context::clear() {
	...
	assert(m_boolSort.id == m_knownTypes[0]->id.id);
	assert(m_intSort.id == m_knownTypes[1]->id.id);
}
...
SortPointer SMTLib2Context::unresolve(SortId _sort) const
{
	smtAssert(_sort.id < m_knownTypes.size());
	auto const& type = *m_knownTypes[_sort.id];
	switch (type.kind)
	{
	case Kind::Int:
	...
	case Kind::BitVector:
	{
		auto it = ranges::find_if(m_bitVectorSorts, [&](auto const& entry) { return entry.second == _sort;});
		smtAssert(it != m_bitVectorSorts.end());
		return std::make_shared<BitVectorSort>(it->first);
	}
	case Kind::Array:
	{
		auto it = ranges::find_if(m_arraySorts, [&](auto const& entry) { return entry.second == _sort; });
	...
	case Kind::Tuple:
	{
		...
		for (auto&& [name, sortId] : tupleType.m_accessors)
		{
			memberNames.push_back(name);
			memberTypes.push_back(unresolve(sortId));
		}
		return std::make_shared<TupleSort>(tupleType.m_name, std::move(memberNames), std::move(memberTypes));
	}
	default:
		smtAssert(false, "Invalid SMT sort");
	}
...
std::string SMTLib2Context::toString(solidity::smtutil::SortId _id)
{
	auto const& sort = m_knownTypes.at(_id.id);
	switch (sort->kind)
	{
	case Kind::Int:
		return "Int";
...
std::optional<std::pair<std::string, SortPointer>> SMTLib2Context::getTupleAccessor(std::string const& _name) const
{
	...
	{
		auto const& type = m_knownTypes.at(sortId.id);
		smtAssert(type->kind == Kind::Tuple);
		auto const& tupleType = dynamic_cast<TupleType const&>(*type);
		for (auto&& [memberName, memberSort] : tupleType.m_accessors)
		{
			if (memberName == _name)
...
void SMTLib2Context::setTupleDeclarationCallback(TupleDeclarationCallback callback)
{
