...
private:
	std::vector<DfsIndex> findDominators(V const& _entry)
	{
		...
		std::vector<std::set<DfsIndex>> predecessors{};
		...
		auto dfs = [&](V const& _v, auto _dfs) -> void {
			...
			semi.emplace_back(dfIdx);
			label.emplace_back(dfIdx);
			...
			ForEachSuccessor{}(_v, [&](V const& w) {
				if (!visited.count(w.id))
				{
					...
					if (dfIdx >= parent.size())
						parent.resize(dfIdx + 1);
					parent[dfIdx] = m_dfsIndexByVertexId[_v.id];
					_dfs(w, _dfs);
				}
				if (m_dfsIndexByVertexId[w.id] >= predecessors.size())
					predecessors.resize(m_dfsIndexByVertexId[w.id] + 1);
				predecessors[m_dfsIndexByVertexId[w.id]].insert(m_dfsIndexByVertexId[_v.id]);
			});
		};
		dfs(_entry, dfs);
		solAssert(dfIdx == m_verticesInDFSOrder.size());
		solAssert(dfIdx == visited.size());
		solAssert(dfIdx == semi.size());
		solAssert(dfIdx == label.size());

		size_t numVertices = visited.size();
		if (predecessors.size() < numVertices)
			predecessors.resize(numVertices);
		if (parent.size() < numVertices)
			parent.resize(numVertices);

		// ancestor(w): The highest ancestor of a vertex ``w`` in the dominator tree used
		// for path compression.
		std::vector<DfsIndex> ancestor(numVertices, std::numeric_limits<size_t>::max());
		...
		std::vector<DfsIndex> idom(numVertices, std::numeric_limits<size_t>::max());
	...
	void buildDominatorTree()
	{
		...
		for (DfsIndex index = 1; index < m_verticesInDFSOrder.size(); ++index)
		{
			...
			m_dominatorTree[idomId].emplace_back(vId);
		}
	}
